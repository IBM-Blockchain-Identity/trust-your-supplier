...
class ConnectionResponder {
	constructor (agent, trusted_connections, interval) {
		...
		if (trusted_connections !== undefined && typeof trusted_connections !== 'string')
			throw new TypeError('Invalid trusted connections string for ConnectionResponder');
				...
				try {
					...
					if (offers.length > 0) {
						...
						try {
							...
							// Any connection request that is not part of our trusted connections list 
							// must be manually accepted.
						} catch (error) {
